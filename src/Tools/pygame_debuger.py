...
def debug(info, y =10, x = PYGAME_CONFIG['WIDTH'] - 250):
    ...
    debug_surf = font.render(str(info), True, COLOR_PALETTE['yellow'])
...
def show_position_vector(screen,vec, text):
    ...
    show_information(f"Pos:{text} <{vec.x:2.2f},{vec.y:2.2f}>", (vec.x - POINT_FEATURES['offset_x'], vec.y + POINT_FEATURES['offset_y']))
