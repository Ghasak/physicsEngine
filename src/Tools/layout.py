...
class Environment(object):
    ...
    def grid(self, selected_color="gray", theta=None, refined=True):
        ...
        for ticks_x in range(self.config["WIDTH"]):
            self.pygame.draw.line(
                ...
                (self.config["WIDTH"], Environment.CONFIG["grid_spacing_x"] * ticks_x),
        ...
        for ticks_y in range(self.config["HIGHT"]):
            self.pygame.draw.line(
                ...
                (Environment.CONFIG["grid_spacing_y"] * ticks_y, self.config["HIGHT"]),
                Environment.CONFIG["grid_line_width"],
            )

        for ticks_x in range(self.config["WIDTH"]):
            # To avoide draw duplicated the font tick on middle of axis
            if int(ticks_x * Environment.CONFIG["grid_spacing_x"]) != (self.hight / 2):
                ...
                debug_rect = font_surf.get_rect(
                    topleft=(
                        (self.width / 2) + Environment.CONFIG["offset_font_x"],
                        Environment.CONFIG["grid_spacing_x"] * ticks_x,
                    )
                )
                pygame.draw.rect(display_surface, self.color["white"], debug_rect)
                display_surface.blit(font_surf, debug_rect)

        for ticks_y in range(self.config["HIGHT"]):
            # To avoide draw duplicated the font tick on middle of axis
            if int(ticks_y * Environment.CONFIG["grid_spacing_y"]) != (self.width / 2):
                display_surface = self.pygame.display.get_surface()
                font_surf = font.render(
    ...
    def draw_vector(
        ...
        if pos is None:
            tail_origin_vector = self.pygame.Vector2(origin_x_new, origin_y_new)
            vector = vector + pos
        else:
            tail_origin_vector = self.pygame.Vector2(
                pos.x + origin_x_new, pos.y + origin_y_new
            )
            self.pygame.draw.line(
                self.surface,
        ...
        if theta is not None:
            transformed_x = vector.x * math.cos(theta) - vector.y * math.sin(theta)
            transformed_y = vector.x * math.sin(theta) + vector.y * math.cos(theta)
            tip_transformed_vector = self.pygame.Vector2(transformed_x, transformed_y)
        else:
...
def Environment_testing_1(
    screen, pygame, PYGAME_CONFIG, COLOR_PALETTE, global_theta, angle
):
    global_theta = 0.0
    env = Environment(screen, pygame, PYGAME_CONFIG, COLOR_PALETTE, global_theta)
